...
**Files:**
- Create: `Context/Sources/ContextMCP/EmbeddingCache.swift`

Note: This lives in ContextMCP (not Context) because the MCP server process is where query embeddings happen.
...
```
git add Context/Sources/ContextMCP/EmbeddingCache.swift
...
### Task 4: Wire QueryPreprocessor + Cache into ContextMCP
...
1. Add QueryPreprocessor import (it's in the Context module — but ContextMCP can't import Context. So we need to **copy the QueryPreprocessor logic into ContextMCP** or make a shared module. The simplest approach: add a lightweight copy of just the `process()` logic into main.swift, similar to how CodeChunk is already duplicated between the two targets.)
...
At the top of main.swift, after the existing model definitions (after `struct IndexState`), add the `QueryPreprocessor` struct. Since ContextMCP is a separate target that can't import the Context module, we duplicate the preprocessor here (same pattern as CodeChunk).
...
```
git add Context/Sources/ContextMCP/main.swift Context/Sources/ContextMCP/EmbeddingCache.swift
...
**Files:**
- Modify: `Context/Sources/ContextMCP/main.swift` (the result formatting section of contextSearch)
...
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"context_search","arguments":{"query":"how does authentication work"}},"id":2}' | .build/debug/ContextMCP
```
...
```bash
echo '{"jsonrpc":"2.0","method":"initialize","params":{"capabilities":{}},"id":1}
{"jsonrpc":"2.0","method":"tools/call","params":{"name":"context_search","arguments":{"query":"BrowserTab.uploadFile"}},"id":2}' | .build/debug/ContextMCP
```
...
| 4 | Wire into ContextMCP (preprocessor + cache + integration) | Modified: `main.swift` |
