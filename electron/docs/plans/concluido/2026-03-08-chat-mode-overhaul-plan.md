...
> **O que foi feito:** CodeFireChat.tsx refatorado de 1956 para ~1250 linhas. Extraídos 4 sub-componentes: ChatHeader.tsx (mode toggle, model selector, conversation dropdown), ChatBubble.tsx (renderização de mensagens, markdown, tools, attachments, actions), ChatInput.tsx (textarea, attachments, paste de imagem, send/cancel), ChatBanners.tsx (rate limit, erro). Funções utilitárias de modelo (CHAT_MODELS, modelHasVision, etc.) movidas para ChatHeader e exportadas.
